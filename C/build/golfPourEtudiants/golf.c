...
/*
  *  paramètre d'entrées  : 	nbParcours
  *				la difficulte sur le green
  *					
  *  en entrées :		le choix du club utilise
  *		
  ...
  *  valeur retournée :		le nombre de coups effectue pour arriver au trou
  *		
  *  DESCRIPTION	: 	Affiche le menu de départ pour savoir ce que le joueur souhaite faire.
  */
...
void afficherInfoTrou( typeTrou unTrou)
{
    ...
    printf("Distance du trou        : %.2f \n", unTrou.distance);
    printf("PAR                     : %d \n", unTrou.par);
    printf("Nombre max de coups    : %d \n", unTrou.nbMaxCoups);
    printf("--------------------------------------------\n");
}
...
void afficherLesParcours( typeParcours *tab[], int nbParcours )
{
    ...
    printf("--------------------------------------------\n");
    printf("Nom     : %s\n",tab[nbParcours]->nom);
    printf("Numéro d'index     : %d\n",tab[nbParcours]->listeTrous[nbParcours].numero);
    printf("--------------------------------------------\n");
...
typeScore *jouerParcours( typeParcours *p )
{
	...
	printf( "Difficulté [0-infernale .. 5-tres facile]" );
	...
	// pour chaque trou du parcours
        int y;
           // afficher les informations concernant le trou 
            printf("Numero du trou : %d\n",p->listeTrous[y].numero);
            printf("Distance du trou : %.2f\n",p->listeTrous[y].distance);
            printf("Par : %d\n",p->listeTrous[y].par);
            printf("Nombre max de coups : %d\n",p->listeTrous[y].nbMaxCoups);
...
/*
  *  paramètre d'entrées  : 	un Parcours
  *				
  *					
  *  en entrées :		
  *		
  ...
  *  valeur retournée :		
  *		
  *  DESCRIPTION	: 	Mode Deux joueurs du golf.
  */
void jouerParcoursADeux( typeParcours *p )
{
    	typeScore *scJ1;
	...
	float niveau;
	// creation d'un nouveau score
	scJ1 = ( typeScore* ) malloc( sizeof( typeScore ) );
        scJ2 = ( typeScore* ) malloc( sizeof( typeScore ) );
	
	printf( "Nom du joueur 1:" );
	scanf( "%s", scJ1->nomJoueur );
        printf( "Nom du joueur 2:" );
	scanf( "%s", scJ2->nomJoueur );
	printf( "Difficulté [0-infernale .. 5-tres facile]" );
	scanf( "%d", &scJ1->difficulte );
        scJ1->difficulte = scJ2->difficulte;
	niveau = 1.0 + 3.0 * ( float ) scJ1->difficulte;
        niveau = 1.0 + 3.0 * ( float ) scJ2->difficulte;
	// recopie des donnees du parcours fait par le joueur
	memcpy( &scJ1->parcoursJoue, p, sizeof( typeParcours ) );
        memcpy( &scJ2->parcoursJoue, p, sizeof( typeParcours ) );
...
/*****************************************************************************************/
	...
	{
		afficherInfoTrou( p->listeTrous[i] );
		scJ1->points[i] = jouerTrou( p->listeTrous[i], niveau );
                scJ2->points[i] = jouerTrou( p->listeTrous[i], niveau );
	}	
